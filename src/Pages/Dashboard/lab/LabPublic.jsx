...
const LabPublic = () => {
  ...
  const divisions = useMemo(
    () => [...new Set(labs.map((l) => l.division).filter(Boolean))],
    [labs],
  );
  ...
  const upazilas = useMemo(
    () => [
      ...new Set(
        labs
          .filter(
            (l) => (!division || l.division === division) && (!district || l.district === district),
          )
          .map((l) => l.upazila)
          .filter(Boolean),
      ),
      ...
      <div className="max-w-7xl mx-auto mt-16 relative z-10">
        ...
        <div className="text-center mb-10">
          <h1 className="text-3xl md:text-4xl font-bold text-emerald-950 mb-2 tracking-tight">
            {t('ICTD Labs')} {t('lab_list_title')}
          </h1>
          <p className="text-emerald-700 max-w-2xl mx-auto">{t('lab_list_subtitle')}</p>
        </div>
        ...
        >
                ...
                <AnimatePresence mode="wait">
                  {paginated.length > 0 ? (
                    paginated.map((l, i) => (
                      ...
                      >
                        <td
                          className={`px-6 font-medium text-emerald-600 ${start + i + 1 === 1 ? 'py-5' : 'py-4'}`}
                        >
                          {start + i + 1}
                        </td>
          ...
          <div className="px-6 py-4 bg-emerald-50/50 border-t border-emerald-100 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-inner">
            <p className="text-sm text-emerald-700">
              {t('showing')} <span className="font-semibold text-emerald-900">{start + 1}</span>{' '}
              {t('to')}{' '}
              <span className="font-semibold text-emerald-900">
                {Math.min(start + entries, filtered.length)}
              </span>{' '}
              {t('of')} <span className="font-semibold text-emerald-900">{filtered.length}</span>{' '}
              {t('entries')}
            </p>
