...
const LabsUnderControl = () => {
  ...
  const lockedDistrictParam =
    !isSuperAdmin && !isDivisionAdmin && !isLabAdmin
  ...
  useEffect(() => {
    const fetchLabs = async () => {
      ...
      try {
        ...
        const divisionParam =
          lockedDivisionParam || (filters.division !== 'All' ? filters.division : null);
        const districtParam =
          lockedDistrictParam || (filters.district !== 'All' ? filters.district : null);
        const upazilaParam =
          lockedUpazilaParam || (filters.upazila !== 'All' ? filters.upazila : null);
  ...
  const currentEntries = labsData.slice(startIndex, startIndex + entriesPerPage);
  ...
  return (
    <div className="min-h-screen bg-emerald-50 p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print">
        <div>
          <h1 className="text-4xl font-bold text-emerald-950">আইসিটিডি ল্যাব </h1>
      ...
      <div className="bg-white backdrop-blur-xl rounded-xl shadow-sm border border-emerald-100 p-5 no-print">
        <div className="flex flex-col gap-6">
          ...
          {/* Filters Row */}
          <div
            className={`grid grid-cols-1 ${isSuperAdmin ? 'sm:grid-cols-2 md:grid-cols-4' : 'sm:grid-cols-2'} gap-4 pt-4 border-t border-emerald-100`}
          >
            ...
            <div
              className={`${isSuperAdmin ? 'sm:col-span-2 md:col-span-4' : 'sm:col-span-2'} flex justify-end`}
            >
              <button
            ...
            <table className="w-full whitespace-nowrap">
              ...
              <tbody className="divide-y divide-emerald-50">
                {currentEntries.length > 0 ? (
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1 items-center space-y-1.5">
                          ...
                          <span className="text-xs text-emerald-400">{startIndex + index + 1}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col max-w-xs">
                          ...
                          <span className="text-xs text-emerald-500 mt-1">আসন: {lab.seat}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col text-sm">
                          {(isSuperAdmin || isDivisionAdmin || isDistrictAdmin) && (
                            <>
                              <span className="text-emerald-800 font-medium">{lab.upazila}</span>
                              {lab.district && (
                                <span className="text-emerald-600 text-xs">{lab.district}</span>
                              )}
                            </>
                          )}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col text-sm">
                          <span className="text-emerald-800 font-medium">{lab.head}</span>
                          <div className="flex flex-col gap-0.5 mt-1">
                            <span className="text-xs text-emerald-500">
                              {formatMobile(lab.mobile)}
                              {lab.altMobile ? `, ${formatMobile(lab.altMobile)}` : ''}
                            </span>
                            {lab.email && (
                              <a
                      ...
                      <td className="px-4 py-2 text-right no-print">
                        <div className="flex items-center justify-center gap-2 ">
                          ...
                          <button
                            ...
                            className={`relative cursor-pointer hover:scale-110 flex items-center gap-2 px-3 py-2 rounded-lg transition-all shadow-sm hover:shadow font-medium text-sm ${
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-8 text-center text-emerald-400">
