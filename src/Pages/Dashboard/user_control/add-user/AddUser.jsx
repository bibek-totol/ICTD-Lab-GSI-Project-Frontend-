...
const AddUser = () => {
  ...
  /* Load BD geo data */
  useEffect(() => {
    fetch(`${API}/data/divisions`, { credentials: 'include' })
      .then((r) => r.json())
      .then((d) => setDivisions(d.divisions || []));
    fetch(`${API}/data/districts`, { credentials: 'include' })
      .then((r) => r.json())
      .then((d) => setDistricts(d.districts || []));
    fetch(`${API}/data/upazilas`, { credentials: 'include' })
      .then((r) => r.json())
      .then((d) => setUpazilas(d.upazilas || []));
  }, []);

  const filteredDistricts = useMemo(
    () => (form.division ? districts.filter((d) => d.division_id === form.division) : []),
    [districts, form.division],
  );

  const filteredUpazilas = useMemo(
    () => (form.district ? upazilas.filter((u) => u.district_id === form.district) : []),
  ...
  const handleEmailBlur = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        const exists = res.data.data.some((u) => u.email?.toLowerCase() === email);
        if (exists) {
      ...
      <div className="max-w-4xl mx-auto mt-5">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-emerald-700">
            Create a new admin with role, designation and jurisdiction
          </p>
        </div>
        ...
        >
            ...
            {requiresDivision && (
              <div>
                ...
                <select
                  ...
                  value={form.division}
                  onChange={(e) =>
                    setForm({ ...form, division: e.target.value, district: '', upazila: '' })
                  }
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-xl text-sm text-blue-700">
            <strong>Note:</strong> Created admins are initially <strong>unverified</strong>.
            SuperAdmin can verify them and assign a password in the Manage Admin section.
          </div>
          ...
          <div className="mt-8 flex justify-end gap-4">
            ...
            <button
              ...
              className={`px-10 py-3 disabled:opacity-70 text-white rounded-xl font-semibold flex items-center gap-2 transition-all ${
                emailError ? 'bg-red-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'
              }`}
            >
              {submitting ? (
                <>
