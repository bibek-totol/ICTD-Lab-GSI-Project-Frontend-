      ...
      const loadGeoData = async () => {
        ...
        try {
          const [res1, res2] = await Promise.all([
            fetch(`${API}/data/srd-data`).then((r) => r.json()),
            fetch(`${API}/data/srd-data300`).then((r) => r.json()),
          ]);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-emerald-600 mt-2">
            Manage all system admins — roles, permissions, and status
          </p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {Object.keys(roleBadge)
                .filter((r) => r !== 'Anonymous' && r !== 'LabAdmin')
                .map((r) => (
        ...
        {totalPages > 1 && (
          <div className="bg-emerald-50/50 px-5 py-4 border-t border-emerald-100 flex items-center justify-between">
            <span className="text-xs text-emerald-600 font-medium font-sans">
              ...
              {Math.min(filtered.length, currentPage * pageSize)} of {filtered.length} admins
            </span>
