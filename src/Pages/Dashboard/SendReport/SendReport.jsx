...
const inspectionReportFields = [
  ...
  {
    label: 'ল্যাব রেনোভেশন/ইন্টেরিয়র ডেকোরেশনের জন্য বরাদ্দ ছিল কিনা? (পরিমাণ)',
...
const getInspectionReportFields = (labType) =>
...
const getInspectionFieldLabels = (field) =>
  field.name === 'sofRoboticsStatus' ? operationLabelHistory : [field.label];
...
const getInspectionFieldType = (fieldName) => {
  ...
  if (['labClassRegister', 'labCameraStatus', 'internetConnectionStatus'].includes(fieldName))
    return 'exists';
  return 'textarea';
};
...
const SendReport = () => {
  ...
  const lockedDistrict =
    !isSuperAdmin && !isDivisionAdmin && !isLabAdmin ? userDistrict || null : null;
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        ...
        const divisionParam =
          lockedDivision || (filters.division !== 'All' ? filters.division : null);
        const districtParam =
          lockedDistrict || (filters.district !== 'All' ? filters.district : null);
        ...
        const reportTypeLabType =
          filters.reportType === 'class_sof'
            ? 'sof'
        ...
        const shouldFetchEquipmentReports =
          filters.reportType === 'All' || filters.reportType === 'equipment';
        const shouldFetchClassReports =
        ...
        setReportsData([
          ...(equipmentResult.data || []).map((report) => ({
            ...report,
            reportType: report.reportType || 'equipment',
          })),
          ...(classResult.data || []).map((report) => ({
            ...report,
            reportType: report.reportType || 'class',
          })),
        ]);
      } catch (error) {
  ...
  const parseInspectionDetails = (report) => {
    ...
    const directDetails = fields.reduce((details, field) => {
      if (
        report[field.name] !== undefined &&
        report[field.name] !== null &&
        report[field.name] !== ''
      ) {
    ...
    fields.forEach((field, index) => {
      ...
      const valueEnd = nextLabel ? summary.indexOf(`${nextLabel}:`, valueStart) : summary.length;
  ...
  const showIctdlClassReports =
    filters.reportType === 'All' || filters.reportType === 'class_ictdl';
  ...
  const ictdlClassReports = showIctdlClassReports
    ...
    : [];
  const shouldRenderEquipmentTable =
    showEquipmentReports &&
    (filters.reportType === 'equipment' ||
      loading ||
  ...
  const currentEntries = equipmentReports.slice(startIndex, startIndex + entriesPerPage);
  ...
  const handleUpdateClassReport = async (event) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (
          selectedReport?.id === editingClassReport.id &&
  ...
  const renderClassReportsTable = (title, reports, keyPrefix, emptyMessage) => {
    const fields = getInspectionReportFields(
      reports[0]?.labType || (keyPrefix.includes('sof') ? 'sof' : 'ictdl'),
    );
    ...
    <div className="min-h-screen bg-emerald-50 p-6 space-y-6">
      ...
      <div className="bg-white backdrop-blur-xl rounded-xl shadow-sm border border-emerald-100 p-5 no-print">
        <div className="flex flex-col gap-6">
          ...
          {/* Filters Row */}
          <div
            className={`grid grid-cols-1 ${isSuperAdmin ? 'sm:grid-cols-2 md:grid-cols-5' : 'sm:grid-cols-2 lg:grid-cols-3'} gap-4 pt-4 border-t border-emerald-100`}
          >
            ...
            {/* Clear Filters */}
            <div
              className={`${isSuperAdmin ? 'sm:col-span-2 md:col-span-5' : 'sm:col-span-2 lg:col-span-3'} flex justify-end`}
            >
              <button
          ...
          <div className="px-6 py-4 border-t border-emerald-100 bg-emerald-50/50 flex flex-col sm:flex-row items-center justify-between gap-4 no-print">
            <div className="flex items-center gap-2 text-sm text-emerald-600">
              ...
              <span className="ml-2 text-emerald-800">
                {startIndex + 1}-{Math.min(startIndex + entriesPerPage, totalEntries)} of{' '}
                {totalEntries}
              </span>
            </div>
      ...
      {showIctdlClassReports &&
        renderClassReportsTable(
          ...
          <div className="print-modal bg-white border-2 border-emerald-100 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="print-modal-content flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="bg-emerald-50/50 backdrop-blur-sm rounded-xl p-5 border border-emerald-100">
                <h3 className="text-lg font-bold text-emerald-800 mb-4">Lab Information</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-emerald-950 ml-2 font-medium">{selectedReport.head}</span>
                  </div>
                  {selectedReport.reportType === 'class' && (
                ...
                <>
                  ...
                  <div className="equipment-screen-sections space-y-6">
                    ...
                    <div className="bg-teal-50/50 backdrop-blur-sm rounded-xl p-5 border border-teal-100">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                          <span className="text-emerald-600 text-xs">🤖 Basic Robotics</span>
                          <p className="text-emerald-950 text-2xl font-bold mt-1">
                            {selectedReport.basicRobotics}
                          </p>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                          <span className="text-emerald-600 text-xs">🦾 Advanced Robotics</span>
                          <p className="text-emerald-950 text-2xl font-bold mt-1">
                            {selectedReport.advancedRobotics}
                          </p>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                          <span className="text-emerald-600 text-xs">🖨️ 3D Printer</span>
                          <p className="text-emerald-950 text-2xl font-bold mt-1">
                            {selectedReport['3dPrinter']}
                          </p>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                          <span className="text-emerald-600 text-xs">🥽 VR Headset</span>
                          <p className="text-emerald-950 text-2xl font-bold mt-1">
                            {selectedReport.vrHeadset}
                          </p>
                        </div>
                        <div className="bg-white p-3 rounded-lg border border-emerald-100 shadow-sm">
                          <span className="text-emerald-600 text-xs">📹 Network Camera</span>
                          <p className="text-emerald-950 text-2xl font-bold mt-1">
                    ...
                    {selectedReport.damageDetails && (
                      <div className="bg-teal-50/50 backdrop-blur-sm rounded-xl p-5 border border-teal-100">
                        <h3 className="text-lg font-bold text-teal-800 mb-3">C. Damage Details</h3>
                      ...
                      <div className="bg-emerald-50/50 backdrop-blur-sm rounded-xl p-5 border border-emerald-100">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {selectedReport.storageImages.map((imageUrl, index) => (
                            <div
                              key={index}
                              className="group relative overflow-hidden rounded-lg border-2 border-emerald-200 shadow-lg hover:shadow-xl transition-all"
                            >
                              <img
                                src={imageUrl}
                                alt={`Storage ${index + 1}`}
                    ...
                    {selectedReport.recommendations && (
                      <div className="bg-teal-50/50 backdrop-blur-sm rounded-xl p-5 border border-teal-100">
                        <h3 className="text-lg font-bold text-teal-800 mb-3">E. Recommendations</h3>
                        <p className="text-emerald-900 leading-relaxed">
                          {selectedReport.recommendations}
                        </p>
                      </div>
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm print:hidden">
          <div className="bg-white border-2 border-emerald-100 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-300">
            <div className="p-6 border-b border-emerald-100 bg-emerald-50/60 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-emerald-950">Update Class Report</h2>
                <p className="text-sm text-emerald-600 mt-1">{editingClassReport.institute}</p>
              </div>
              <button
                type="button"
                onClick={() => {
                    ...
                    >
                      ...
                      {fieldType === 'number' ? (
                        <input
                          ...
                          onChange={(event) =>
                            handleClassReportFormChange(field.name, event.target.value)
                          }
                        ...
                        <>
                          <div className="flex flex-wrap gap-3">
                            {choiceOptions.map((option) => (
                              <label
                                key={option}
                                className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-semibold text-emerald-800"
                              >
                                <input
                                  type="radio"
                                  name={field.name}
                          ...
                          <textarea
                            ...
                            onChange={(event) =>
                              handleClassReportFormChange(detailName, event.target.value)
                            }
